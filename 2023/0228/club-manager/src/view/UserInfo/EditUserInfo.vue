            ...
            <el-form :model="formInfoData" label-width="120" :rules="formInfoDataRules" ref="formEl">
                <el-form-item label="用户头像" prop="user_photo">
                    <el-upload v-loading="isUploadingImg" element-loading-text="正在上传图片"
                        :action="baseURL + '/userInfo/uploadUserPhoto'" :headers="{ authorization: store.token }"
                        name="user_photo"
                        ...
                        <el-image :src="baseURL + formInfoData.user_photo" v-if="formInfoData.user_photo" fit="contain"
                ...
                <el-form-item label="所在学院" prop="user_institute">
                    <el-select v-model="formInfoData.user_institute" placeholder="请选择所在学院">
                        <el-option value="信息工程">信息工程学院</el-option>
...
onMounted(() => {
    formInfoData.value.id = route.params.id;
    API.userInfo.findById(formInfoData.value.id)
        .then(result => {
            formInfoData.value = result;
        })
})
...
const uploadSuccess = (result) => {
    formInfoData.value.user_photo = result.data;
...
const addUserInfo = () => {
    ...
    API.userInfo.update(formInfoData.value)
        .then(result => {
            ElNotification({
                type: "success",
                title: "保存成功",
